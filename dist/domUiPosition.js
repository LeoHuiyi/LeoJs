...
/***/ function(module, exports, __webpack_require__) {
	...
	var _domUiPositionJs = __webpack_require__(31);

	var positionable = _domUiPositionJs.leoDom.$('.positionable');

	function position() {
	    _domUiPositionJs.leoDom.uiPosition(positionable, {
	        of: _domUiPositionJs.leoDom.$("#parent"),
	        my: _domUiPositionJs.leoDom.$val("#my_horizontal") + " " + _domUiPositionJs.leoDom.$val("#my_vertical"),
	        at: _domUiPositionJs.leoDom.$val("#at_horizontal") + " " + _domUiPositionJs.leoDom.$val("#at_vertical"),
	        collision: _domUiPositionJs.leoDom.$val("#collision_horizontal") + " " + _domUiPositionJs.leoDom.$val("#collision_vertical"),
	        within: window
	    });
	}
	...
	var defaultOption = {
	    elem: '',
	    drag: _domUiPositionJs.leoDom.noop,
	    eventName: 'leoDrag'
	};
	...
	var Drag = (function () {
	    function Drag() {
	        ...
	        this.option = _domUiPositionJs.leoDom.extend({}, defaultOption, option);
	        this.elem = _domUiPositionJs.leoDom.$(this.option.elem);
	    ...
	    _createClass(Drag, [{
	        ...
	        value: function init() {
	            var isDrag = false;
	            var elem = this.elem;
	            var drag = this.option.drag;
	            var eventName = this.option.eventName;

	            _domUiPositionJs.leoDom.$on(elem, "mousedown." + eventName, function (event) {
	                event.preventDefault();

	                var offset = _domUiPositionJs.leoDom.$offset(this);
	                var cLeft = event.pageX - offset.left;
	                var cTop = event.pageY - offset.top;

	                isDrag = true;

	                _domUiPositionJs.leoDom.$on(document, 'mousemove.' + eventName, function (event) {
	                    event.preventDefault();

	                    if (isDrag) {
	                        var left = event.pageX - cLeft;
	                        var _top = event.pageY - cTop;

	                        _domUiPositionJs.leoDom.$offset(elem, {
	                            left: left,
	                            top: _top
	                        });
	                ...
	                _domUiPositionJs.leoDom.$on(document, 'mouseup.' + eventName, function (event) {
	                    event.preventDefault();

	                    if (isDrag) {
	                        _domUiPositionJs.leoDom.$off(document, '.' + eventName);
	        ...
	        value: function destroy() {
	            _domUiPositionJs.leoDom.$off(this.elem, '.' + this.option.eventName);
	        }
	    }]);
	...
	_domUiPositionJs.leoDom.$css(positionable, "opacity", 0.5);
	_domUiPositionJs.leoDom.$on("select, input", "click keyup change", position);
...
/***/ function(module, exports, __webpack_require__) {
	...
	*/
	...
	var _offsetJs = __webpack_require__(26);
	...
	supportsOffsetFractions = function () {
	    var elem = _offsetJs.leoDom.$('<div>'),
	        support = undefined;

	    _offsetJs.leoDom.$css(elem, "position", "absolute");
	    _offsetJs.leoDom.$offset(elem, {
	        top: 1.5,
	        left: 1.5
	    });

	    _constJs.document.body.appendChild(elem[0]);
	    support = _offsetJs.leoDom.$offset(elem).top === 1.5;
	...
	function parseCss(element, property) {
	    return parseInt(_offsetJs.leoDom.css(element, property), 10) || 0;
	}
	...
	function getDimensions(elem) {
	    ...
	    if (raw.nodeType === 9) {
	        return {
	            width: _offsetJs.leoDom.$width(elem),
	            height: _offsetJs.leoDom.$height(elem),
	            offset: {
	                top: 0,
	                left: 0
	            }
	    ...
	    if (_offsetJs.leoDom.isWindow(raw)) {
	        return {
	            width: _offsetJs.leoDom.$width(elem),
	            height: _offsetJs.leoDom.$height(elem),
	            offset: {
	                top: _offsetJs.leoDom.$scrollTop(elem),
	                left: _offsetJs.leoDom.$scrollLeft(elem)
	            }
	        };
	    }
	    ...
	    return {
	        width: _offsetJs.leoDom.$outerWidth(elem),
	        height: _offsetJs.leoDom.$outerHeight(elem),
	        offset: _offsetJs.leoDom.$offset(elem)
	    };
	}

	_privateJs._leoDom.setApi(_offsetJs.leoDom, {
	    scrollbarWidth: function scrollbarWidth() {
	        ...
	        var w1 = undefined,
	            w2 = undefined,
	            div = _offsetJs.leoDom.$("<div style='display:block;position:absolute;width:50px;height:50px;overflow:hidden;'><div style='height:100px;width:auto;'></div></div>"),
	            innerDiv = _offsetJs.leoDom.$children(div)[0];

	        _offsetJs.leoDom.$append("body", div);
	        w1 = innerDiv.offsetWidth;
	        _offsetJs.leoDom.$css(div, "overflow", "scroll");
	        ...
	        _offsetJs.leoDom.$remove(div);

	        return cachedScrollbarWidth = w1 - w2;
	    },

	    getScrollInfo: function getScrollInfo(within) {
	        var overflowX = within.isWindow || within.isDocument ? "" : _offsetJs.leoDom.$css(within.element, "overflow-x"),
	            overflowY = within.isWindow || within.isDocument ? "" : _offsetJs.leoDom.$css(within.element, "overflow-y"),
	            hasOverflowX = overflowX === "scroll" || overflowX === "auto" && within.width < within.element[0].scrollWidth,
	            hasOverflowY = overflowY === "scroll" || overflowY === "auto" && within.height < within.element[0].scrollHeight;

	        return {
	            width: hasOverflowY ? _offsetJs.leoDom.scrollbarWidth() : 0,
	            height: hasOverflowX ? _offsetJs.leoDom.scrollbarWidth() : 0
	        };
	    },

	    getWithinInfo: function getWithinInfo(element) {
	        var withinElement = _offsetJs.leoDom.$(element || window),
	            isWindow = _offsetJs.leoDom.isWindow(withinElement[0]),
	        ...
	        return {
	            ...
	            offset: hasOffset ? _offsetJs.leoDom.$offset(element) : {
	                left: 0,
	                top: 0
	            },
	            scrollLeft: _offsetJs.leoDom.$scrollLeft(withinElement),
	            scrollTop: _offsetJs.leoDom.$scrollTop(withinElement),
	            width: _offsetJs.leoDom.$outerWidth(withinElement),
	            height: _offsetJs.leoDom.$outerHeight(withinElement)
	        };
	    },
	    ...
	    positionCollision: {
	        ...
	        flipfit: {
	            left: function left() {
	                _offsetJs.leoDom.positionCollision.flip.left.apply(this, arguments);
	                _offsetJs.leoDom.positionCollision.fit.left.apply(this, arguments);
	            },
	            top: function top() {
	                _offsetJs.leoDom.positionCollision.flip.top.apply(this, arguments);
	                _offsetJs.leoDom.positionCollision.fit.top.apply(this, arguments);
	            }
	        }
	    },
	    ...
	    uiPosition: function uiPosition(selector, options) {
	        ...
	        options = _offsetJs.leoDom.extend({}, options);
	        ...
	        var atOffset = undefined,
	            ...
	            target = _offsetJs.leoDom.$(options.of),
	            within = _offsetJs.leoDom.getWithinInfo(options.within),
	            scrollInfo = _offsetJs.leoDom.getScrollInfo(within),
	        ...
	        _offsetJs.leoDom.$(selector).forEach(function (elem) {
	            var collisionPosition = undefined,
	                using = undefined,
	                elemWidth = _offsetJs.leoDom.$outerWidth(elem),
	                elemHeight = _offsetJs.leoDom.$outerHeight(elem),
	                ...
	                position = _offsetJs.leoDom.extend({}, basePosition),
	                myOffset = getOffsets(offsets.my, _offsetJs.leoDom.$outerWidth(elem), _offsetJs.leoDom.$outerHeight(elem));
	            ...
	            _offsetJs.leoDom.$offset(elem, _offsetJs.leoDom.extend(position, {
	                using: using
	            }));
	        });
	    }
	...
	exports.leoDom = _offsetJs.leoDom;
